...
class Hive:
    ...
    def run_cycle(self):
        ...
        for bee in self.bees:
            ...
            # CORRECTION: Utiliser generate_signal() si disponible (bee_signal_generator)
            # Sinon utiliser analyze_market() (méthode originale)
            signal = None
            confidence = 0.0
            
            if hasattr(bee, 'generate_signal'):
                # Utiliser le générateur de signaux (plus souple)
                try:
                    signal_data = bee.generate_signal()
                    if signal_data:
                        signal = signal_data.get('type', 'HOLD')
                        confidence = signal_data.get('confidence', 0) / 100.0  # Convertir en 0-1
                except Exception as e:
                    logger.error(f"Error in generate_signal for {bee.bee_id}: {e}")
                    signal = 'HOLD'
                    confidence = 0.0
            else:
